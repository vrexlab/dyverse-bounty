...
const expectThrow = async promise => {
  try {
    ...
    const VMException = err.message.includes("revert");
    assert(
...
const fullMatrixOfAllowed = async function() {
  const nKydys = (await coreC.totalSupply()).toNumber();
  console.log("__fullMatrixOfAllowed:", nKydys);
  for (let i = 1; i <= nKydys; i++) {
    for (let j = 1; j <= nKydys; j++) {
      try {
...
const blockNumber = async () => {
  ...
  return new web3.BigNumber(res['result']);
}
